...
### traceroute에 대해 간략하게 조사하고 실습

`traceroute`

- 발신지에서 목적지까지 가는 동안 거치는 라우터들의 IP와, 각 구간 지연시간을 측정하는 네트워크 진단 도구
- TTL 값을 1부터 차례로 증가시키며 패킷을 전송하고, 각 홉에서 TTL이 0이 될 때 되돌아오는 ICMP Time Exceeded 메시지를 이용해 경로를 추적한다.
- 일부 라우터는 ICMP 응답을 차단할 수 있어 결과가 * 로 표시되거나, 비대칭 라우팅 환경에서는 실제 왕복 경로와 출력이 다를 수 있다.

### 라우팅 테이블에 대한 이론

### 실제 ubuntu에서의 라우팅 테이블 실습

vmbr0의 vm새로 하나 생성하시고, vmbr0의 dhcp가 아닌 별개의 vm과 vmbr1만을 가진 vm이 서로 ping이 갈 수 있도록 dhcp 서버의 라우팅테이블과 각각 엔드포닌트의 라우팅 테이블도 잡을 수 있도록 해보세요.